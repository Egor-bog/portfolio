...
fetch('./pets.json').then(res => res.json()).then(list => {
    ...
    createPets(fullPetsList);
     ...
     for (let i = 0; i < (fullPetsList.length / 6); i++) {
         ...
         for (let j = 0; j < 6; j++) {
             stepList.forEach((item, int) => {
                 if (item.name === stepList[j].name && (int !== j) ) {
...
const sort6recursively = (list) => {
    ...
    for (let i = 0; i < (length / 6) ; i++) {
        ...
        for (let j = 0; j < 6; j++) {
            let dublicatedItem = stepList.find((item, ind) => {
...
document.querySelector("#prevPage").addEventListener('click', () => {
    ...
    document.querySelector('#pets').style.top = `calc(35px - ${190 * pageCounter}px)`;
});

document.querySelector("#nextPage").addEventListener('click', () => {
    if (pageCounter < (document.querySelector('#pets').offsetHeight / 190 - 1)) {
        pageCounter ++; 
    }
    document.querySelector('#pets').style.top = `calc(35px - ${190 * pageCounter}px)`;
});